...
//set the chat text to the previous message whenever the Tab key is pressed, ensuring the chat box recalls the last message
                        if (key === 9 && this.chatTyped.length === 0 && this.chatHistoryIndex > 0) {
                            this.chatTyped = this.chatHistory[--this.chatHistoryIndex];
                            this.redrawChatback = true;
                        } else if (key === 9 && this.chatTyped.length === 0 && this.chatHistoryIndex < this.chatHistory.length - 1) {
                            this.chatTyped = this.chatHistory[++this.chatHistoryIndex];
                            this.redrawChatback = true;
                        }

                        // custom: when holding enter, clicking makes your character instantly go to the location
                        if (
                            key === 13 &&
                            this.mouseClickButton === 1 &&
                            typeof this.mouseClickX === 'number' &&
                            typeof this.mouseClickY === 'number' &&
                            this.mouseClickX >= 550 && this.mouseClickX <= 695 &&
                            this.mouseClickY >= 4 && this.mouseClickY <= 155
                        ) {
                            // Calculate the clicked position on the minimap
                            let x: number = this.mouseClickX - 25 - 550;
                            let y: number = this.mouseClickY - 4 - 4;

                            if (x >= 0 && y >= 0 && x < 146 && y < 151 && this.localPlayer) {
                                x -= 73;
                                y -= 75;
                                ...
                                // Send move command to the calculated position
                                this.out.p1isaac(ClientProt.MOVE_MINIMAPCLICK);
                                this.out.p1(5); // some flag
                                this.out.p2(tileX);
                                this.out.p2(tileZ);
                                this.out.p1(this.actionKey[5] === 1 ? 1 : 0); // run flag

                                this.flagSceneTileX = tileX;
                                this.flagSceneTileZ = tileZ;
                            }

                            // Clear the chat if there was text
                            if (this.chatTyped.length > 0) {
                                this.chatTyped = '';
                                this.redrawChatback = true;
                            }
                            
                            // Prevent normal Enter key processing for this special case
                            break;
                        }

                        // custom: when holding enter, clicking in the game viewport makes your character instantly go to the location
                        if (
                            key === 13 &&
                            this.mouseClickButton === 1 &&
                            typeof this.mouseClickX === 'number' &&
                            typeof this.mouseClickY === 'number' &&
                            this.mouseClickX > 4 && this.mouseClickY > 4 &&
                            this.mouseClickX < 516 && this.mouseClickY < 338
                        ) {
                            // Use the game's built-in coordinate conversion system
                            this.scene?.click(this.mouseClickX - 8, this.mouseClickY - 11);
                            
                            // Check if World3D detected a valid tile click
                            if (World3D.clickTileX !== -1 && this.localPlayer) {
                                const tileX: number = World3D.clickTileX;
                                const tileZ: number = World3D.clickTileZ;
                                
                                // Reset the click coordinates since we're handling it
                                World3D.clickTileX = -1;
                                
                                // Send teleport command to the calculated position
